/**
 * Runs a performance load test simulating multiple concurrent virtual users.
 * @param {object} config - Load test configuration: { url, method, headers, body, vusers, iterations }
 * @param {function} onProgress - Progress callback (completed, total)
 */
export async function runLoadTest(config, onProgress) {
  const { url, method, headers = {}, body, vusers = 5, iterations = 10 } = config;
